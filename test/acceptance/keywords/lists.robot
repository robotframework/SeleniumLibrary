...
List Selection Should Be
    [Documentation]    LOG 2 Verifying list 'interests' has no options selected. LOG 5 Verifying list 'possible_channels' has option(s) [ email | Telephone ] selected.
...
Select From Single Selection List
    ...
    #do something else... anything to ensure the list is really set as the next keyword will pass if list item is highlighted but not selected
    Unselect from List    possible_channels
    List Selection Should Be    preferred_channel    Telephone
...
Select Non-Existing Item From Single Selection List
    [Tags]    OnlyThisOne
    Run Keyword And Expect Error    ValueError: Option 'Smoke Signals' not in list 'preferred_channel'.    Select From List    preferred_channel    Tin Can Phone    Smoke Signals
    Select From List    preferred_channel    Tin Can Phone    Smoke Signals    Email
    Run Keyword And Expect Error    ValueError: Option 'Tin Can Phone' not in list 'preferred_channel'.    Select From List    preferred_channel    Smoke Signals    Email    Tin Can Phone
    Run Keyword And Expect Error    NoSuchElementException: Message: Could not locate element with visible text: Tin Can Phone\    Select From List By Label    preferred_channel    Tin Can Phone

Select Non-Existing Item From Multi-Selection List
    [Tags]    OnlyThisOne
    Run Keyword And Expect Error    ValueError: Options 'Tin Can Phone, Smoke Signals' not in list 'possible_channels'.    Select From List    possible_channels    Tin Can Phone    Smoke Signals
    Run Keyword And Expect Error    ValueError: Options 'Tin Can Phone, Smoke Signals' not in list 'possible_channels'.    Select From List    possible_channels    Tin Can Phone    Smoke Signals    Email
    Run Keyword And Expect Error    ValueError: Options 'Tin Can Phone, Smoke Signals' not in list 'possible_channels'.    Select From List    possible_channels    Tin Can Phone    Email    Smoke Signals
